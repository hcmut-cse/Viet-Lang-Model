...
with open(os.path.join(DATA_PATH, 'VNTQcorpus-small.txt')) as f:
    listLines = f.read().split('\n')

listSens = []
for line in listLines:
    sentToken = sent_tokenize(line)
    for sent in sentToken:
        listSens.append(sent)
        print(sent)
print("Sentences Tokenized Complete!")

data = []
for sen in listSens:
    wordToken = word_tokenize(sen)
    for word in wordToken:
        data.append(word)
        print(word)

print("Words Tokenized Complete!")
...
import torch
torch.save(tokenizer, os.path.join(DATA_PATH, 'viet_model_tokenizer.h5'))

...
data = []
if os.path.exists(WORD_SAVE):
    with open(WORD_SAVE, 'r', encoding='utf8') as f:
        data = f.read().split('\n')
else:
    with open(os.path.join(DATA_PATH, 'viwiki-latest-pages-articles.txt')) as f:
...
with open(WORD_SAVE, 'w', encoding='utf8') as f:
    for word in data:
        f.write(word + '\n')
...
import torch
torch.save(tokenizer, os.path.join(DATA_PATH, 'viet_wiki_tokenizer.h5'))
